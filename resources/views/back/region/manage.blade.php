    ...
    <div class="grid grid-cols-1 gap-4 h-fit">
        ...
        <div class="app-plate">
            <div class="mb-5">
                ...
                <p class="text-slate-700">Regional Head</p>
            </div>
        ...
        <div class="app-plate">
            ...
            <p class="text-sm text-slate-700">Additional information is not available. To appoint a regional head, please select from the list of managers.</p>
        </div>
        @endif
        <div class="app-plate">
            <h1 class="text-lg mb-2">Regional Activity Log</h1>
...
<script>
    $(document).ready(() => {
        let table = new CustomDataTable({
            ...
            'module': 'user',
            'language': {
                'emptyTable': `No managers assigned. Ask the national officer to add one regional officer here.`
            },
            'config': (d) => {
                d.config = {
                    role: 'regional',
            ...
            'columns': [{
                ...
                },
                {
                    ...
                    render: (data, type, row) => {
                        ...
                        if (REGIONAL_HEAD_ID == row['id']) return '';

                        return `<div class="dropdown">
                      <div perms='regional.assign_head'><div tabindex="0" role="button" class="btn btn-sm m-1">Action</div>
        ...
        $(document).on('click', '.fn-assign-head', (e) => {
            ...
            makeModal(`assign-head`, `Assign "${name}" as head of ${REGION_NAME}?`, () => {
                is_sending = true;
                $.ajax({
                    type: 'PUT',
                    url: `${BASE_URL}/back/region/${DATA_ID}`,
                    data: {
                        head_user_id: id
                    },
                    success: (e) => {
        ...
        $(document).on('click', '.fn-remove-head', (e) => {
            ...
            makeModal(`remove-head`, `<b>Resign</b> "${name}" as head of ${REGION_NAME}?`, () => {
                is_sending = true;
                $.ajax({
                    type: 'PUT',
                    url: `${BASE_URL}/back/region/${DATA_ID}`,
