<script>
    let DATA_ID = "{{$id}}";
    let MUNICIPAL_ID = "{{$data['municipal_id']}}";
    let BARANGAY_NAME = "{{$data['name']}}";
    let BARANGAY_HEAD_ID = "{{$head['id'] ?? ''}}"
</script>
<div class="mb-10">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
    <div class="grid grid-cols-1 gap-4 h-fit">
        ...
        <div class="app-plate">
            <div class="mb-5">
                ...
                <p class="text-slate-700">Province Head</p>
            </div>
            ...
            @can('barangay.assign_head')
            <button class="fn-remove-head btn btn-neutral mt-4 btn-sm" data-name="{{$head['name'] ?? ''}}">
                Remove
            </button>
            @endcan
        </div>
        ...
        <div class="app-plate">
            ...
            <p class="text-sm text-slate-700">Additional information is not available. To appoint a Province head, please select from the list of managers.</p>
        </div>
        @endif
        <div class="app-plate">
            <h1 class="text-lg mb-2">Province Activity Log</h1>
    ...
    <div class="lg:col-span-2 ">
        <div class="app-plate overflow-auto lg:overflow-visible">
            <table id="table">
                <thead>
                    <tr>
                        <th>Managers</th>
...
<script>
    $(document).ready(() => {
        let table = new CustomDataTable({
            ...
            'module': 'user',
            'language': {
                'emptyTable': `No managers assigned. Ask the national officer to add one barangay director here.`
            },
            'config': (d) => {
                d.config = {
                    role: 'barangay',
            ...
            'columns': [{
                ...
                },
                {
                    ...
                    render: (data, type, row) => {
                        ...
                        if (BARANGAY_HEAD_ID == row['id']) return '';

                        return `<div class="dropdown">
                      <div perms='barangay.assign_head'><div tabindex="0" role="button" class="btn btn-sm m-1">Action</div>
        ...
        $(document).on('click', '.fn-assign-head', (e) => {
            ...
            makeModal(`assign-head`, `Assign "${name}" as head of ${BARANGAY_NAME}?`, () => {
                is_sending = true;
                $.ajax({
                    type: 'PUT',
                    url: `${BASE_URL}/back/barangay/${DATA_ID}`,
                    data: {
                        head_user_id: id
                    },
                    success: (e) => {
                    ...
                    error: (e) => {
                        if (e.responseJSON) {
                            makeModal('error', e.responseJSON?.message ?? "unable to assign head", () => {});
                        } else {
                            makeModal('error', "Something went wrong", () => {});
                        }
                    },
                    complete: () => {
                        is_sending = false;
        ...
        $(document).on('click', '.fn-remove-head', (e) => {
            var name = $(e.target).attr('data-name');

            if (is_sending) return;

            makeModal(`remove-head`, `<b>Resign</b> "${name}" as head of ${BARANGAY_NAME}?`, () => {
                is_sending = true;
                $.ajax({
                    type: 'PUT',
                    url: `${BASE_URL}/back/barangay/${DATA_ID}`,
