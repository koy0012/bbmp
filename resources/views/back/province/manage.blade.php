<script>
    ...
    let PROVINCE_NAME = "{{$data['name']}}";
    let PROVINCIAL_HEAD_ID = "{{$head['id'] ?? ''}}"
</script>
<div class="mb-10">
...
<script>
    $(document).ready(() => {
        let table = new CustomDataTable({
            ...
            'module': 'user',
            'language': {
                'emptyTable': `No managers assigned. Ask the national officer to add one provincial officer here.`
            },
            'config': (d) => {
                d.config = {
                    role: 'provincial',
            ...
            'columns': [{
                ...
                },
                {
                    ...
                    render: (data, type, row) => {
                        ...
                        if (PROVINCIAL_HEAD_ID == row['id']) return '';

                        return `<div class="dropdown">
                      <div perms='provincial.assign_head'><div tabindex="0" role="button" class="btn btn-sm m-1">Action</div>
        ...
        $(document).on('click', '.fn-assign-head', (e) => {
            ...
            makeModal(`assign-head`, `Assign "${name}" as head of ${PROVINCE_NAME}?`, () => {
                is_sending = true;
                $.ajax({
                    type: 'PUT',
                    url: `${BASE_URL}/back/province/${PROVINCE_ID}`,
                    data: {
                        head_user_id: id
                    },
                    success: (e) => {
        ...
        $(document).on('click', '.fn-remove-head', (e) => {
            ...
            makeModal(`remove-head`, `<b>Resign</b> "${name}" as head of ${PROVINCE_NAME}?`, () => {
                is_sending = true;
                $.ajax({
                    type: 'PUT',
                    url: `${BASE_URL}/back/province/${PROVINCE_ID}`,
